...
`setup.sh` 会准备内置 Node.js runtime、安装依赖、缓存离线文档转换资源并拉取默认工作区 `mino`；重复运行会复用已校验的文档资源缓存。如果你没有 GitHub SSH key，`openmino` 的 SSH clone 可能失败；可以先配置 GitHub SSH，或手动把默认工作区准备到仓库根目录的 `mino/` 后重新运行。
...
`setup.sh` prepares the bundled Node.js runtime, installs dependencies, caches the offline document-processing resources, and pulls the default `mino` workspace. Repeated runs reuse the verified document-resource cache. If you do not have a GitHub SSH key, the SSH clone for `openmino` may fail. Configure GitHub SSH first, or prepare the default workspace manually under `mino/` and rerun the script.
