...
try {
    ...
    if (-not $cliproxyDistribution) { $cliproxyDistribution = Join-Path $ProjectDir "src-tauri\resources\cliproxy-cache\distribution" }
    node "$ProjectDir\scripts\package-cliproxy-component.mjs" stage --from $cliproxyDistribution --platform win32-x64
