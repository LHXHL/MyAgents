...
- Markdown 链接使用 Accent，不显示常驻下划线；明确的文件/网页以文件类型图标/链接图标提供非颜色识别。文件复用 `FileIcon` 图标资源，使用 1em 尺寸和相对基线跟随标题、正文及表格字号，按真实目标而非展示标题分类。hover/focus 与右键复制保留完整原始引用（相对/绝对路径、行号或 URL），实际打开时才解析目标，长地址在有界提示框内换行，点击立即收起。可点击代码文件名在正文、列表和表格中统一去掉底色，普通非链接代码保留原样。普通未确认文件的行内代码保留 code 外观；引用使用 `ink-secondary` 和结构边界，不降低到不可读灰度。
