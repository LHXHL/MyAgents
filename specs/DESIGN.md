...
展开态 256px:
  ...
  工作区行: h-9；14px 展开箭头的可见左缘对齐分区标题，分支线穿过箭头中心 + text-sm 名称；整行只负责展开/折叠；顶层条目额外行间距 0
...
- 顶层工作区按连续资源树排布，工作区 wrapper 之间不添加额外 gap；工作区行与 Session 行都使用 `h-9`。普通工作区的展开箭头固定 14px：工作区列表使用 8px 左 inset，按钮不再叠加左 padding，使 Lucide 箭头的可见笔画左缘落在分区标题的 12px 左边界；展开分支边线相对列表内容使用 6px 左缩进，使 1px 线的中心与箭头中心落在同一视觉轴。工作区图标、名称和 Session 分支跟随该树轴自然左收，为资源名释放横向空间。工作区名称与 Session 标题都是主要可点击资源名，统一使用 `text-sm` 14px；层级由缩进、图标、颜色和字重表达。来源 tag 与日期保留 `text-xs` 12px meta 档，不能与主标题等权。工作区名称默认 `font-normal`，仅在 hover、focus、菜单打开、Launcher 关联或包含当前 Session 时升至 `font-medium`，避免静态资源列表持续争夺注意力。
