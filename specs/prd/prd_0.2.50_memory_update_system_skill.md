...
- [x] Phase 3：将默认 `UPDATE_MEMORY.md` 模板改为 frontmatter-only，移除失效 placeholder plumbing，保留既有文件不覆盖。
- [x] Phase 3 验证：缺失文件创建、既有文件不覆盖、零字节/frontmatter-only ready、symlink/目录 fail closed。
...
- 2026-07-13：完成 Phase 3。默认 `UPDATE_MEMORY.md` 改为 frontmatter-only，正文默认为空；Renderer 与 Rust 缺失文件创建路径均直接使用同一共享模板。删除 `{{MEMORY_RULE_PATH}}` 以及 TS/Rust 渲染函数。新增并通过缺失创建、既有自定义不覆盖、零字节/frontmatter-only ready、目录与 symlink fail-closed 测试；Rust memory_auto_update 16 项、memory_rules 3 项、typecheck 与 prompt builder 测试通过。
