...
- 侧栏是独立的 App Shell material consumer：根面只读取完整 Theme 必需的 `--global-sidebar-bg`，该 Token 由每套 Theme 的 light/dark package 拥有。页面根面与卡片/弹层继续使用既有 `--paper / --paper-elevated / --paper-inset` 语义；顶部 Tab 栏属于右侧 Workspace，根面使用纯 `--paper`，Tab active/hover 复用全局 `--hover-bg`，active 额外以 `--accent` 底线表达注意力。不能为制造左右差异而翻转通用 Paper 层级、在组件内混色或为 Tab Chrome 复制一套局部 palette。
