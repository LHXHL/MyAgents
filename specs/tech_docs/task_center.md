...
- Management API：`/api/task/*`（含 comment/list/context/retry、turn admitted、trigger validate/test/check-now/reset、run-now，以及 canonical start/stop/runs）及 App 内兼容 `/api/cron/*`。精确 Task ID route 直接交给 TaskStore adapter，不套 ambient workspace guard。
