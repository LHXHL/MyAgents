...
脚本复用 `.env` 凭证、R2 bucket、`download.myagents.io` 域名、Cloudflare purge 和上传后 HTTP 验证。正式上传仍要求 `scripts/package-managed-codex-runtime.mjs` 完成 manifest/artifact 签名校验；它会逐个枚举 Mach-O / PE 并校验签名。macOS 主 `codex` 与 `codex-code-mode-host` 必须保持 OpenAI Team/Developer ID；仅对固定路径且确认为完全未签名的 `rg` / `zsh` helper，正式打包才会用 `.env` 中的 `APPLE_SIGNING_IDENTITY` 补签并复验，动作写入 `release-audit-v1.json`。开发用 unsigned 包只应使用 `npm run package:managed-codex` 本地生成，不应上传到正式 R2 路径。
