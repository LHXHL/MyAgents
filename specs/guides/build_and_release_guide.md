...
脚本复用 `.env` 凭证、R2 bucket、`download.myagents.io` 域名、Cloudflare purge 和上传后 HTTP 验证。正式上传仍要求 `scripts/package-managed-codex-runtime.mjs` 完成 manifest/artifact 校验；它会逐个枚举 Mach-O / PE，固定 native 文件集合，并校验上游签名。macOS 主 `codex`、`codex-code-mode-host` 及 voice host / 动态库必须保持 OpenAI Team/Developer ID；原生路径清单位于 `scripts/package-managed-codex-policy.js`，voice 目录不使用通配放行，新增或缺失 Mach-O 都会阻断打包。完整保留上游 voice manifest、runtime、sources、NOTICE 和 licenses，打包支持不代表启用产品语音 UI；macOS `rg` / `zsh` helper 若已有同一 OpenAI Developer ID 或旧版 ad-hoc 签名则原样保留并复验，只有固定路径且确认为完全未签名时才使用 `.env` 中的 `APPLE_SIGNING_IDENTITY` 补签。Windows 的 OpenAI 原生二进制必须通过 Authenticode 且 publisher 默认为 `OpenAI OpCo, LLC`，实际 signer certificate SHA-256 只写入 `release-audit-v1.json`，不作为每个版本都要人工更新的发布锁；Windows `codex-path/rg.exe` 保持上游未签名状态并写入 audit。开发用 unsigned 包只应使用 `npm run package:managed-codex` 本地生成，不应上传到正式 R2 路径。
