...
export function SpaceAvatar({
  ...
  const fallbackTextClass = size >= 32 ? 'text-sm' : 'text-xs';
  ...
  return (
    <span className={`inline-grid shrink-0 place-items-center rounded-full border border-[var(--line-subtle)] bg-[var(--paper-inset)] ${fallbackTextClass} font-semibold text-[var(--ink-muted)] ${className}`} style={style}>
...
export function SpaceIdentityLine({
  ...
  return (
    <span className={`inline-flex min-w-0 items-center gap-1.5 ${className}`}>
      ...
      <span className={`min-w-0 truncate ${nameClassName}`}>{label}</span>
    </span>
  );
}
