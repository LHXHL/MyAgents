...
function FilePatchSection({
  ...
  const path = pathPresentation(change.path, t);
  ...
  return (
    ...
    >
      <header className="flex min-h-12 items-center gap-2.5 border-b border-[var(--line-subtle)] px-3 py-2">
        ...
        <div className="min-w-0 flex-1" title={change.path}>
          <div className="flex min-w-0 items-center gap-1.5">
            <h3 id={headingId} className="truncate font-mono text-sm font-semibold text-[var(--ink)]">
              {path.fileName}
            </h3>
            {movePath && (
              <>
      ...
      <FilePatchViewport change={change} fileName={path.fileName} t={t} />
    </section>
  );
}
...
function FilePatchViewport({
  ...
  const source = change.rows.map((row) => row.kind === 'hunk' || row.kind === 'omission' ? '' : row.text).join('\n');

  return (
    <div
      className="overflow-x-auto bg-[var(--paper)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--focus-border)]/25"
      tabIndex={0}
...
function pathPresentation(path: string | undefined, t: ChatTranslator): { fileName: string; parent: string } {
  if (!path?.trim()) return { fileName: t('shell.toolChrome.filePatch.unnamedFile'), parent: '' };
  const normalized = path.replace(/\\/g, '/').replace(/\/$/, '');
  const parts = normalized.split('/').filter(Boolean);
  const fileName = parts.pop() || t('shell.toolChrome.filePatch.unnamedFile');
  return { fileName, parent: parts.slice(-2).join('/') };
}
