...
describe('bridge-registry — Responses prompt_cache_key downgrade state', () => {
  it('keeps the unsupported-field downgrade on same-token re-register and clears it on unregister', () => {
    ...
    expect(isResponsesPromptCacheKeyDisabled('tok-cache')).toBe(false);

    disableResponsesPromptCacheKey('tok-cache');
    expect(isResponsesPromptCacheKeyDisabled('tok-cache')).toBe(true);
