...
export const SESSION_ENGINE_ROUTE_CONTRACTS: SessionEngineRouteContract[] = [
  ...
  },
  {
    ...
    failureStatuses: [500],
    behavior: 'Starts a new desktop conversation through the active engine; external owns subprocess cleanup.',
  },
  {
    path: '/chat/rewind',
    method: 'POST',
    ...
    failureStatuses: [400],
    behavior: 'Rewinds builtin sessions through the engine; external runtimes return an unsupported capability error.',
  },
  {
    ...
    responseKeys: ['success', 'content', 'attachments', 'error'],
    failureStatuses: [400],
    behavior: 'Retries the latest external user message through the external engine; builtin returns an unsupported capability error.',
  },
  {
    path: '/chat/stream',
    method: 'GET',
    engineMethod: 'getStreamReplaySnapshot',
    responseKeys: ['chat:init', 'chat:message-replay', 'chat:logs', 'chat:system-init'],
    failureStatuses: [],
    behavior: 'Initial SSE replay comes from the active engine snapshot; SSE disconnect remains non-cancelling.',
  },
  {
  ...
  },
  {
    ...
    method: 'POST',
    engineMethod: 'runInjectedTurn',
    requiredFields: ['goalId', 'objective', 'sessionId', 'queueId', 'expectedControlRevision'],
    responseKeys: ['success', 'outputText', 'sessionId', 'goalChannelDeliveryExpected', 'error'],
    failureStatuses: [400, 408, 409, 500, 503],
    behavior: 'Claims the supplied queue turn at a clean boundary and waits for true engine completion.',
  },
  {
    path: '/api/im/enqueue',
    method: 'POST',
  ...
  },
  {
    ...
    responseKeys: ['success', 'runtime', 'model', 'mcpServerIds', 'agentNames', 'permissionMode', 'reasoningEffort'],
    failureStatuses: [500],
    behavior: 'Reads the active engine configuration snapshot without route-level runtime branching.',
  },
  {
  ...
  },
  {
    ...
    failureStatuses: [],
    behavior: 'Reads the latest assistant result from the active engine with runtime-specific fallback hidden in adapters.',
  },
  {
    path: '/api/session-watch/register',
    method: 'POST',
    engineMethod: 'getRuntimeIdentity/getLiveSessionState/getLatestAssistantResult',
    ...
    behavior: 'Registers session watches against the active engine identity and state.',
  },
  {
    path: '/sessions/:id',
    method: 'GET',
    ...
    behavior: 'Merges persisted session history with active engine live overlay for the requested session.',
  },
  {
    path: '/sessions/fork',
    method: 'POST',
  ...
  },
  {
    ...
    failureStatuses: [500],
    behavior: 'Creates a new published IM session through the active engine; external owns stop/rebind sequencing.',
  },
  {
    ...
    failureStatuses: [400, 500],
    behavior: 'Applies desktop interaction scenario only where the active engine supports it.',
  },
  {
    path: '/api/mcp/set',
    method: 'POST',
  ...
  },
  {
    ...
    responseKeys: ['success', 'skipped', 'error'],
    failureStatuses: [500],
    behavior: 'Applies builtin agent overrides through the engine; external runtimes explicitly skip route-level mutation.',
  },
  {
    ...
    responseKeys: ['success', 'error'],
    failureStatuses: [500],
    behavior: 'Applies provider env through the engine while preserving the legacy success-only route response.',
  },
  {
