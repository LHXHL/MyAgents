...
- **排队发送时保持正确的停止状态**：排队消息自动开始或点击“立即发送”后，等待回复期间持续显示停止按钮，避免短暂切回发送按钮；主窗口与桌宠保持一致。
- **连续对话中的停止更可靠**：上一轮停止确认尚未返回时，仍可停止新一轮；迟到的确认或超时不会误停后续对话。
- **执行错误不再被误当作取消**：停止过程中发生真实执行失败时，保留正确的错误提示与结果，正常完成的后续对话也不再误标为已停止。

---

## [0.4.19] - 2026-09-19

> MyAgents 0.4.19 提升会话保存、回溯、分叉与重试的可靠性，改善问答、定时任务和渠道操作，并升级内置 Claude Agent SDK。

### Changed

- **内置 Claude Agent SDK 升级至 0.3.276**：恢复会话时正确采用更新后的产品提示词，适配后台任务完成通知，并支持工具审批的默认拒绝和禁止始终允许约束；不同来源的 MCP 工具分别处理授权。

### Fixed

- **会话保存更可靠**：修复回溯后无法继续保存的问题，避免慢磁盘读取导致历史被误判为空或损坏。
- **回溯、分叉与重试保留准确上下文**：恢复和继续对话时保持选定的历史边界，分叉继承原会话配置；慢磁盘下等待实际完成，打开分支失败不再丢弃已创建的会话。
- **重试使用当前模型选项**：Codex 重试遵循界面所选模型与推理强度，并正确保留其他排队消息。
- **工作区列表更耐受异常记录**：个别 Agent 或工作区记录缺失身份、发生冲突时，其他正常工作区仍可显示和打开。
- **修改配置不再覆盖其他选择**：调整权限或推理强度时保留已更新的模型；切换运行时后，页面打开失败不撤销已保存配置或已创建会话。
- **停止与新会话状态更准确**：避免迟到响应误改执行状态；新空会话保存模型配置，重启后可恢复，并可直接创建当前会话任务。
- **定时任务状态与历史更一致**：启动失败保留任务和错误，等待触发时不误显示正在输出，停止后可恢复提示词；切换会话时，迟到的任务结果不再混入其他会话。
- **渠道连接失败可见、可停止**：IM 渠道启动失败显示错误，异常但仍启用的渠道可停止并取消自动重连。
- **问答提交失败保留答案**：主窗口和桌宠中的问题在提交成功后才收起，失败后可重试，旧问题的迟到回执不会清除新问题。
- **聊天显示与连接更稳定**：文件链接重新校验时减少闪烁和排版跳动，附件变化及时刷新，并修复页面重新挂载时的聊天事件连接问题。

### Known Issues

- 拒绝文件写入后，聊天中的编辑统计仍可能显示已修改文件；实际写入会被阻止。
- 刚启用 MCP 服务时，当前会话可能到下一轮对话才提供对应工具。

---

## [0.4.18] - 2026-09-17

> MyAgents 0.4.18 新增 Markdown 文档大纲导航，改善长文档编辑、流式对话阅读及录音转写与播放体验，更新 DeepSeek Flash 默认模型和 Codex 推理选项，并提供 Ubuntu 24.04 x64 源码构建入口。

### Added

- **Markdown 文档大纲**：悬停即可查看标题层级和当前位置，点击标题快速跳转，也可通过键盘选择章节。
- **Ubuntu 24.04 x64 源码构建**：提供环境初始化、开发版和 deb 安装包构建入口及验收指南；当前仍需真实桌面验收，托管订阅、桌宠和桌面控制等功能尚未齐平。

### Changed

- **流式对话跟随更平滑**：持续输出和内容高度变化时平稳跟随底部，手动滚动即可停止跟随，便于随时回看；保留减少动态效果设置。
- **DeepSeek 默认模型升级**：预设默认使用支持图片输入的 DeepSeek V4.1 Flash，保留用户已保存的模型选择。
- **Codex 推理选项跟随模型能力**：托管 Codex 显示当前模型实际支持的推理等级，切换模型时使用相应默认值，减少不兼容选项。
- **行内文件图标更协调**：调整图标与文字的大小和基线，改善聊天、表格及文件链接的阅读效果。

### Fixed

- **实时转写更连贯**：减少连续讲话被错误切成极短片段的问题，结束静音调整为 1 秒，兼顾句子完整性与出字等待。
- **录音结束和再次录制更顺畅**：避免上一段录音的后台收尾阻止开始新录音，减少已处理暂停录音结束时的重复等待，并更清楚地显示后续处理阶段。
- **录音播放恢复正常**：修复进度前进却没有声音的问题，改善双轨合听、跳转和播放进度同步；点击转写、笔记或标记定位后，可直接用空格播放或暂停。
- **录音界面适配夜间模式**：统一录音与播放器的深色背景、文字和控件对比，停止按钮保持红底白字。
- **源码构建更耐受慢网络、更节省磁盘**：延长资源下载等待并重试临时网络故障，复用已验证缓存，减少重复下载和开发编译产物占用，并提供明确的缓存查看与清理入口。
- **长 Markdown 文档编辑更稳定**：减少滚动时的排版跳动和位置偏移，改善图片、HTML 换行与长表格的显示和定位。
- **表格操作更易触达**：鼠标移向复制或下载按钮时工具栏保持可用，复制成功后显示明确反馈。
- **完成状态不再反复闪动**：已完成的 Agent 状态面板只收起一次，减少聊天底部和输入区反复抖动。
- **长会话打开后正确定位最新消息**：修复部分 macOS 环境加载长历史时停在较早消息、需要再次回到底部的问题，同时保持长消息缩短后的滚动范围正确。
- **标签页达到上限时明确提示**：从不同入口打开会话或工作区时给出一致反馈，避免点击后没有响应。
- **订阅登录失败原因更清晰**：区分授权拒绝、超时和临时查询故障，避免重复提交登录回调，改善取消后的状态处理。

---

## [0.4.17] - 2026-09-13

> MyAgents 0.4.17 新增 Antigravity 订阅接入，提升 Markdown 阅读、表格和文件链接体验，并改善长对话保存、录音转写与说话人识别。

### Added

- **Antigravity 订阅接入**：可在客户端连接 Google Antigravity 账户、获取模型列表并直接对话，所需组件由应用自动准备与维护。
- **表格复制与 Excel 下载**：悬停表格即可复制整表或下载 `.xlsx` 文件；复制同时提供富文本表格和制表符文本，方便粘贴到云文档、文本编辑器及电子表格。
- **已完成录音可重新转写**：在录音详情菜单中重新处理已有录音，处理期间保留旧稿；成功后更新结果，并尽可能保留有效的人物标注。

### Changed

- **Markdown 阅读与编辑样式统一**：加深正文与标题颜色、调整字重和排版间距，统一聊天、预览与直接编辑中的表格、引用、代码及其他内容样式。
- **宽表格更易阅读**：宽窗口下利用正文两侧留白，窄窗口下按内容保留合适列宽并在表格内横向滚动，减少日期和短字段被挤成竖排；表格滚动优先于顶部标签切换。
- **文件链接更清晰**：本地文件链接复用文件类型图标，图标随文字缩放；悬停显示原始链接，右键可复制原始地址，去除常驻下划线并统一正文和表格中的表现。
- **Codex 权限与原生模式对齐**：外部 Codex 提供 Ask for approval、Approve for me 和 Full Access；托管 Codex 的“行动”和“自主行动”分别对应自动审查与完全访问，“规划”保持原有行为。
- **录音来源与说话人识别改善**：更准确地处理麦克风和系统声音的时间差、回声及同时发言；人物归属不再等同于音轨，证据不足时保留“未确定”。
- **已安装语音模型自动维护**：应用可在后台更新已安装的模型资源，更新期间保留兼容旧版供现有任务使用，下载失败不打断当前可用能力。

### Fixed

- **会话创建、恢复与分支更可靠**：修复部分 Codex 新会话无法打开、分支后无法继续的问题；改善长对话流式保存、恢复、搜索及多入口读取，减少重复正文和附件预览丢失。
- **异步提问可保留并继续回答**：支持在聊天与小助理中选择选项或补充文字，恢复会话后保留问题与已接纳的回答状态，排队或发送失败不再误标为已回答。
- **本地文件预览兼容更多链接形式**：统一处理相对路径、绝对路径、文件 URL、行号及编码字符，改善工作区外文件和符号链接的预览与右键操作。
- **悬停提示与菜单不再遮挡操作**：点击功能后及时收起提示，文件树右键菜单可正确显示在文档预览上方；用户消息气泡采用侧栏同色背景，聊天分隔更清晰。
- **减少流式日志占用**：不再逐条落盘高频正文增量和状态快照，保留汇总信息及必要的错误记录。
...
- **Markdown 打开即可编辑**：工作区 Markdown 默认将排版与编辑合为一体，支持格式工具栏、查找替换和选区引用；切换全屏或源码模式时保留内容、选区和撤销历史。
- **直接编辑表格与插入图片**：可编辑表格单元格、增删行列、调整对齐并粘贴多格数据；拖入或粘贴的图片自动保存到文档同级图片目录，正文使用相对引用。
- **逐行处理文件冲突**：文档自动保存，检测到外部修改时暂停覆盖；可上下比较本地草稿与磁盘内容，逐行选择并检查组合结果，也可将草稿另存副本。保存失败时保留编辑内容。
- **失败的 MCP 工具可在会话内重连**：内置 AI 与托管 Codex 会话可查看工具连接失败原因，并在空闲时重试，无需修改配置或新建对话。

### Changed

- **会话搜索按最近活动完整分页**：搜索结果按会话汇总，按最近活跃时间排序并持续加载，显示完整匹配总数；长对话不再挤掉其他匹配会话。
- **Grok 订阅默认模型更新为 4.6**：新增对应模型能力和推理等级选项，同时保留已有模型选择与旧账户兼容。
- **侧栏和工作区操作更清晰**：改善文字层级、选中与悬停状态，统一聊天操作和文件工具栏尺寸，优化文件搜索摘要与窄窗口布局。

### Fixed

- **聊天与文档中的本地图片、链接正确解析**：修复本地图片无法显示，以及外部文档相对引用错位的问题，兼容中文、编码字符、文件 URL 和 Windows 路径。
- **流式对话滚动更稳定**：持续输出时平稳跟随新内容；向上阅读、搜索或展开详情后保留阅读位置，迟到的布局变化不再拉回旧位置。
- **中文输入与会话标签操作更可靠**：输入法组词期间避免误发送、误选择或关闭界面；切换会话后标签菜单正确更新，旧请求不再覆盖新选择。
- **录音结束后的转写与失败恢复更可靠**：修复部分双轨录音自动及手动转写失败的问题，改善连续长段语音处理；结果保存失败时保留原始音频和已有结果，并提供更明确的原因与重试入口。
- **浏览器取消、重连和登录状态保存更稳定**：避免取消或关闭与仍在执行的操作互相干扰，改善重连后的页面管理，以及多个会话间登录状态的保存。
- **微信插件消息监听恢复兼容**：兼容微信插件新版接口，修复注册和扫码登录成功后仍无法开始监听消息的问题。
- **插件安装和工具启动更可靠**：减少误用系统 Node/npm 导致的安装与启动失败，失败时保留实际错误原因。
- **文件和评论定位更准确**：文件树完成布局后再定位搜索目标；任务评论跳转只滚动评论阅读区域。
- **减少隐藏界面的后台活动**：音频播放结束及设置预览关闭时及时释放资源，隐藏的聊天状态栏停止不必要的计时刷新和动画。
- **桌面显示与编辑兼容性改善**：修复部分 WebKit 环境创建浏览器后的界面报错，提高 Windows 窗口和托盘图标清晰度，并改善 macOS 编辑器的撤销与重做。

---

## [0.4.15] - 2026-09-06

> MyAgents 0.4.15 接入词元跳动，支持一键连接账户、查看余额与扫码充值；同时更新桌面操作能力，改善供应商展示和对话计时。

### Added

- **词元跳动模型供应商**：可在客户端发起网页登录授权，自动完成账户连接，查看余额并使用微信或支付宝扫码充值；内置多家厂商的模型预设，也可发现和添加更多支持的模型。
- **按模型自动选择接入协议**：词元跳动的预设和新增模型都会根据支持情况选择合适协议，使用不同厂商模型时无需手动配置。

### Changed

- **供应商展示更清晰**：词元跳动固定在启用列表首位，提供模型官网入口和随主题变化的微光背景；所有供应商卡片标题增大一档。
- **桌面操作能力随客户端更新**：macOS 与 Windows 的 Cuse 改为内置 Skill，可在全局 Skills 中启停；升级会更新工具内容并保留关闭状态。
- **内置 Node/npm 环境更新**：统一使用 Node 24.20.0 与 npm 11.19.0 的官方发行组合，确保随应用打包的版本一致。

### Fixed

- **对话耗时不再随界面更新重置**：工具执行、提示变化或切换页面时持续累计当前回合耗时，等待用户确认期间暂停计时。
- **Windows PowerShell 工具稳定启用**：内置 AI 对话可同时使用 Bash 和 PowerShell，不再依赖上游分批开放状态。
...
- **Session 支持全局多标签整理**：可以为一个 Session 添加多个标签，并统一创建、重命名、合并或删除标签；标签会跨工作区汇总，可直接点击筛选，也能与关键词搜索组合使用。
- **侧边栏可直接重命名和置顶 Session**：重命名结果会同步到已打开的对话与历史搜索；置顶顺序会持久保留，常用会话更容易找到。

### Changed

- **Session 资源菜单更聚焦常用操作**：复制 Session ID 置于菜单首位，重命名、置顶和标签整理保持在侧边栏与历史搜索中一致。
- **源码构建会先检查原生依赖**：macOS、Windows 与 Linux 构建在下载或编译大型资源前检查所需的 CMake、Python 等环境，并提供可执行的修复提示；中断后的 macOS ONNX Runtime 缓存也可以安全恢复。

### Fixed

- **从对话定位文件夹不再引起工作区抖动**：右侧文件树已打开时会直接展开并高亮目标文件夹，不再先关闭后重新打开；Windows 非系统盘中的已登记工作区也可以正常定位。
- **Codex 回合边界的后续消息不再丢失**：在上一轮刚结束时发送的新消息会按原顺序进入下一轮，不再因过期的实时注入状态被撤回或遗漏，修复 [#565](https://github.com/hAcKlyc/MyAgents/issues/565)。
- **图片理解启用流程保持真实状态**：首次启用必须先选择可用模型并保存，取消或保存失败时开关继续保持关闭；发送图片时仅在图片理解尚未就绪且工具未启用的情况下提示，修复 [#561](https://github.com/hAcKlyc/MyAgents/issues/561)。
- **内嵌浏览器关闭与重开更稳定**：旧页面遗留的创建、关闭和回调不会再影响新打开的浏览器；登录状态保存异常也不会阻塞旧浏览器资源退出。
- **桌面宠物在隐藏或关闭后完整释放资源**：Companion、原生窗口和动画播放会随真实生命周期停止，空闲、隐藏、完成及减少动态效果状态下不再持续占用渲染资源。

---

## [0.4.12] - 2026-08-31

> MyAgents 0.4.12 新增完整的本地录音与离线语音识别工作流：可以同时录制麦克风与系统声音，实时查看转写，在 Record 中播放、整理说话人、记录笔记，并把结果交给 AI 继续讨论。版本还支持在对话中离线转写本地音视频附件，并让 OCR 与语音模型在共享推理资源下保持前台优先和有界回收。

### Added

- **完整的音频 Record 与录音工作流**：可以选择麦克风、系统声音或两者同时录制，暂停后继续，录制时实时查看文本；完成后可播放麦克风、系统与混合音轨，并在统一的 Record 列表中管理文字与录音记录。
- **完全离线的实时转写与会后整理**：语音识别、本地补全和说话人区分均在设备上完成；录音结束后会自动收敛为更完整的逐字稿，并支持重命名、合并说话人或调整单条文本归属。
- **对话内本地音视频附件转写**：Agent 可以通过内置语音识别能力异步转写当前工作区中的单个音频或视频文件，查询进度、取消任务，并获得 Markdown 与结构化结果，全程不依赖在线语音服务。
- **可管理的本地语音模型资源**：设置页可以查看、安装、校验和移除离线语音模型；模型资源与应用版本解耦，按需下载并由应用统一管理。

### Changed

- **Record 成为文字与录音的统一信息入口**：快速启动器与任务中心共用一致的记录入口；录音详情集成逐字稿、播放器、笔记、标记和导出，完成的内容可以直接发起 AI 讨论。
- **实时录音优先使用本地推理资源**：OCR、录音转写、会后处理和后台资源校验由统一调度协调；实时任务需要资源时，批处理会主动让出，后台校验也不会阻塞 App 启动。
- **模型生命周期兼顾响应速度与内存兜底**：暂停录音时保留实时模型 10 分钟以便快速继续，超时后自动释放；任务结束与应用退出都有明确的协作停止和最终清理上界。

### Fixed

- **macOS 双架构与 Windows 首次构建恢复可靠**：同时构建 Apple Silicon/Intel 时，各架构只使用自己的本地推理资源；Windows 初始化也不再因上游源码包中的无关链接文件、过长临时路径或错误的 ONNX Runtime 导入库配置而中断。
- **Record 搜索结果更容易判断命中内容**：音频记录会直接展示搜索命中的文本片段，批量操作入口也与当前界面保持一致。
- **录音权限、设备变化与保存后转写更可靠**：权限检查、输入设备变化和短暂采集异常都有明确状态与恢复路径；结束保存后会生成可完整解码的录音归档并继续会后补全，不再因媒体尾部边界被误判为损坏。
- **停止录音后最终转写会自动完整出现**：会后补全结束时，当前详情页会直接从实时草稿切换到最终结果，无需关闭后重新打开；同时避免麦克风和系统声音录到相同内容时在最终稿中重复出现。
- **实时转写、播放与详情交互更稳定**：修复实时文本延迟出现、录音尚未完成时过早暴露播放、说话人操作错位，以及长内容下卡片和操作区域不稳定的问题。
- **长录音和本地 Worker 可以有界收敛**：补全、说话人处理、OCR 与资源校验在暂停、取消、超时或退出时都会释放对应代际，避免后台进程和模型长期占用资源。
...
- **Task 智能创建与本地协作**：创建任务时可以先与 AI 多轮讨论，由 AI 在最终确认前生成完整 `task.md` 和触发参数；任务详情以内容视图展示正文、调度信息、执行 Session 与线性评论，评论可继续发送到最近或指定 Session，Agent 回复也会回写到任务。
- **Task 评论通知与精确跳转**：Agent 在本地 Task 中回复后会进入统一通知中心；点击应用内或系统通知可直接打开对应任务并定位评论，已读状态在重启后保持一致。
- **MyAgents 托管浏览器**：新增独立于 Playwright MCP 的“浏览器”工具，由应用管理 Chromium、登录状态和 Session 隔离；全局可以同时启用两种工具，具体对话中按需互斥选择，避免重复暴露浏览器能力。
...
- **安全安装本地 Skills**：可以从本地目录或压缩包安装 Skill；安装前会限制读取范围、校验结构并在失败时回滚，避免把目录外文件或半成品写入能力库。
- **统一通知中心与应用内路由**：侧边栏新增常驻通知入口，统一展示官方公告、Space Issue 评论与本地 Task 回复；严格版本化的 `myagents://open/v1/...` 深链负责精确导航，附件资源则迁移到独立的 `myagents-resource://` 协议并保留历史兼容。
- **飞书 Bot 扫码配置**：Agent Channel 向导支持通过二维码完成飞书应用授权与凭据配置，并保留手动填写方式。

### Changed

- **Task 创建、列表和详情体验统一**：侧边栏任务行支持快捷创建；智能与手动创建共用一个全局面板，手动的一次性任务移除无关的周期策略；列表按进行中、待恢复、已完成和规划中组织，详情页保持正文、评论输入与右侧属性信息的稳定布局。
- **Managed Codex 升级至 0.149.0**：继续使用原有已适配能力，不自动开放新增实验接口；0.146 创建的健康历史 Session 可直接续跑，不再因协议版本或 Host 工具目录指纹变化要求新建会话。
- **Builtin 工具与群聊能力更一致**：Builtin 会话可识别并展示 Agent、MultiEdit 和 PowerShell 工具；群聊不再默认隐藏 Bash、Edit 与 Write，仍保留 Channel 权限、群审批、MCP 开关和显式工具禁用配置。
...
- **Task 执行与评论链路保持正确 Session**：任务派发、重新派发、评论排队和 Session 结束现在按各自 owner 收敛；长任务正文、Markdown 评论、引用回复和任务通知不会再错位、溢出或跳转失效。
- **浏览器工具可靠启动并保持正确身份**：应用统一管理 Playwright 与托管浏览器的资源安装、Host 启动准入和 Product Session capability，修复宿主缺失、错误 Session 身份、虚假设置入口及工具数量按子工具膨胀的问题。
- **长对话滚动与中途注入更稳定**：窗口隐藏、恢复、尺寸变化和历史加载不再破坏阅读锚点；强制发送到正在执行的回合会立即显示已接纳消息，终端错误也会保留 Provider 返回的关键详情。
- **Space 与 Agent 设置写入更安全**：MCP 工具编辑统一回到配置 owner，Registered Agent 设置使用磁盘内合并，避免并发更新覆盖其它字段。
...
- **任务支持 Command Detector 条件触发**：定时检查先运行本地命令，只有命令明确返回匹配结果时才创建 Session 并唤醒 AI；未命中时保持安静且不消耗模型额度，同时提供配置校验、测试运行、立即检查、超时与有界诊断。
- **Codex 对话支持回退与分支**：可以从指定用户消息回退当前对话，或保留原会话并创建分支；文件恢复结果、会话身份和后续消息会落到正确的目标会话。
- **新增 MyAgents Light 主题**：新安装默认使用更明亮、克制的产品主题；已有用户明确选择过的主题保持不变。

### Changed

- **任务中心与 Agent 自动化体验更完整**：创建任务可自动推断当前工作区，列表、详情、触发状态和失败诊断使用一致的生命周期语义；CLI 与内置自动化 Skill 补齐 Detector 的创建、验证、检查和治理入口。
- **第三方 Provider 预设跟进新模型**：Google Gemini 默认更新为 `gemini-3.6-flash`；Moonshot 默认使用 `kimi-k3` 并保留 K2.6 与 K2.7 Code；Kimi Code 增加 1M 与 256K 的 K3；智谱精简为 GLM 5.2 与 GLM 5 Turbo；MiniMax 精简为 M3、M2.7 与 M2.7 Highspeed。
- **全局侧栏补充对话入口**：在跨页面导航时可更直接地新建或回到对话，工作区和会话状态继续保持独立。

### Fixed

- **长上下文展示与自动压缩使用同一能力来源**：模型上下文按 Provider 精确解析，歧义结果安全拒绝；界面与运行时统一按 90% 阈值提示和触发自动压缩，避免显示值与真实行为不一致。
- **配置与 MCP 并发变更更可靠**：配置在权威锁内重新读取并合并，跨窗口刷新、全局 MCP 增删、重复添加和模型选择不会再被过期状态覆盖。
- **Session 与外部 Runtime 的持久化边界更稳**：Provider 路由、配置快照、MCP authority、转录尾部变更与隐藏 SDK 工具遵循同一 Session owner，降低恢复、重置和并发入口造成的串写或能力丢失。
- **跨平台进程收尾与错误诊断更准确**：Linux 的 Detector 管道提前关闭和僵尸进程不再误判为执行失败或残留进程；SDK 启动错误保留实际来源平台，便于给出正确诊断。
...
- **CLI 支持按 Agent 发起异步 Session 协作**：可以发现 Agent / Workspace，并向指定 Agent 创建新的异步 Session；消息接纳与结果回传沿用现有 Inbox 和 Session 生命周期，适合脚本与其他 Agent 调用。

### Changed

- **Managed Codex 升级至 0.146.0**：模型目录、权限模式和运行来源由 Agent、Session 与 Channel 的真实配置统一决定；响应 Token 用量按 Provider 返回值精确汇总，避免子 Agent 或缓存基线造成误计。
- **恢复的会话直接成为可用 Chat Tab**：批量恢复会立即挂载真实会话并隔离单个失败项，不再先创建空壳、等用户逐个点开后才初始化。
- **任务创建与更新更一致**：绑定当前会话的定时任务会先完成 Session 身份落盘；CLI 的通知字段在权威锁内合并，并使用真实接纳序号记录执行，减少关闭 Tab 或并发编辑造成的配置丢失。

### Fixed

- **Sidecar 重启与后台任务恢复更可靠**：进程替换、失败候选、全局服务恢复和通知完成状态按精确进程代际隔离；旧响应不会命中新进程，已有任务和会话 owner 会在有界重试中保留。
- **并发入口不再覆盖外部 Runtime 消息**：桌面、IM、任务、Inbox 和注入消息各自保留正确的用户消息身份，拒绝或取消只撤回自己的未落盘内容，不会产生幽灵气泡或串写其他请求。
- **流式阅读与折叠交互更稳定**：窗口失焦但仍可见时继续实时渲染；返回应用会保留跟随底部或历史阅读位置。展开/折叠较长消息时由同一虚拟列表维护几何，减少跳动、错位和点击区域滞后。
- **本地通信与大响应更有界**：单个 SSE 事件、代理响应和落盘引用均有明确大小及并发边界；写入采用原子、无覆盖协议，异常输入不会无限增长内存或磁盘占用。
- **后台进程退出更彻底**：Sidecar 与 Plugin Bridge 从进程出生起持有精确子进程树，正常退出、升级和强制清理不会遗漏后代进程或误杀无关命令。
- **Chat 与窄屏体验细节修复**：窄窗口优先保留工作区名称并自适应任务控件；Tooltip 不再被滚动容器裁切，链接右键菜单会关闭文本选择菜单，连续点击段落时也不会出现整行异常选区。

---

## [0.4.2] - 2026-07-30

> MyAgents 0.4.2 聚焦历史会话恢复与内容预览：已有 Session 会在独立 Tab 中稳定打开或复活，恢复期间只展示一次最终内容，侧边栏历史也会随磁盘变化实时刷新。版本同时统一网页与文件链接动作，补齐图片和文档预览的缩放手势，并收紧 Sidecar 重启、应用退出和消息持久化边界。

### Added

- **图片预览支持完整缩放与平移手势**：支持双击缩放、触摸板捏合、双指平移、鼠标拖拽、滚轮与工具栏控制，缩放锚点和边界约束保持一致；PDF、DOCX 与 PPTX 预览也支持跨 WebView 的触摸板捏合缩放。

### Changed

- **历史 Session 统一按独立 Tab 打开**：已打开的会话直接聚焦，Sidecar 已停止时复用原 Tab 并自动复活，其余会话从首帧创建目标 Tab；不再把当前 Tab 的 Runtime 热切换到另一段历史。
- **网页与文件链接使用统一预览动作**：Chat 中网页链接默认进入内嵌浏览器，Cmd/Ctrl 点击交给系统浏览器；可预览文件在产品内打开，工作区目录定位到文件树，不存在或不支持的目标会给出明确反馈。
- **侧边栏历史会话实时更新**：桌面、IM、定时任务、CLI 或迁移写入 Session 后，新增、标题、状态和删除会自动同步到已加载的工作区列表，无需重新打开搜索或重启应用。

### Fixed

- **历史恢复不再闪现原始内容或重复加载**：冷 Tab、REST 历史与实时事件由同一个恢复壳层衔接，最终内容只揭示一次；持续事件缺口或恢复失败会隔离不可信内容，并保留明确的重试路径。
- **Sidecar 重绑与重启不再串会话**：进程管理身份与业务 Session 身份分离，pending materialization、桌面重置和已确认迁移不会破坏管理权限；进程替换会重建正确的实时 revision 基线，同时保留 Task 与 Tab owner。
- **退出、重置与首条消息持久化更可靠**：应用生命周期由 Rust 统一清理 Sidecar，React 重挂载不会误杀进程；会话 transcript 在 Runtime dispatch 前提交，并以每 Session 文件锁串行多入口写入，降低退出或并发操作造成的上下文丢失风险。
...
- **全局侧边栏统一产品导航**：新对话、搜索、任务、团队、技能与工具、Agent 工作区和历史 Session 在 Launcher、Chat 与功能页面间持续可见；支持 256px 展开态、64px 折叠 rail、工作区悬浮列表和独立的用户收展偏好。
- **技能与工具成为独立功能 Tab**：技能、插件和工具迁入单实例页面，与设置页分别保留自己的导航、草稿和弹层状态，重复打开会直接聚焦已有 Tab。

### Changed

- **Launcher 与工作区资源各归其位**：Launcher 收敛为创建新工作的单列入口；工作区、Session 历史、置顶、归档、设置、打开文件夹和新建对话统一由全局侧边栏承载，展开侧栏与 rail 悬浮列表保持相同菜单和键盘顺序。
- **历史搜索打开更快、筛选更轻**：历史 Overlay 会先展示现有快照并虚拟化长列表，冷启动不再二次闪现；非搜索状态只保留“全部 / 收藏”和工作区筛选，浏览、全文搜索与 Session ID 命中共用同一套右键菜单。
- **输入框权限和低频动作更清晰**：builtin、Claude Code、Codex 与 Gemini 的权限模式统一使用线性边界图标；“定时任务”移入 `+` 菜单，`+`、权限和工具菜单统一从下向上展开，文件引用与技能选择弹窗使用更克制的阴影。
- **设置与任务创建进一步减负**：网络代理提升为独立设置页并继续复用原有持久化与热更新链路；任务创建不再要求手工标签，从想法派发时仍会保留已有标签来源。
- **侧边栏与 Tab 层级更稳定**：侧栏使用各主题独立的结构背景，工作区展开具有平滑纵向动画；选中、生成中与未读状态在侧栏和顶部 Tab 间保持一致，折叠/展开时主内容与左右工作区同步归位。

### Fixed

- **绑定 IM 的外部 Runtime 回复恢复可靠投送**：桌面发起的 Claude Code、Codex 和 Gemini 回合会在真实成功并持久化后把完整回复发送到绑定 Channel；用户消息镜像失败、自动重试、停止或空回复不会造成错投、乱序或重复发送。
- **Codex 多级子 Agent 不再混入主对话**：兼容新旧 Codex 协作协议，子 Agent 的文本、思考、工具和附件会稳定归入对应协作卡片；停止和强制发送会等待或中断真实子回合，不再提前结束主任务。
- **统一日志显著减噪并保持可诊断**：SSE 流式 delta、Claude Code raw partial 和重复的 Node/Rust 输出不再落盘；完成文本只记录单行前 100 字预览。Codex 系统指令保持脱敏，Bridge 功能异常只记录有界状态并不会误重启仍存活的插件进程。
- **macOS 窗口缩放后原生控件不再卡位**：红绿灯位置由 NSWindow 几何通知持续维护，覆盖双击缩放、拖拽调整、全屏和显示缩放变化；64px rail 下品牌、功能图标与原生控件使用同一视觉轴。
- **Space 同名 Skill 更新不再静默改名**：检测到同名 Skill 时默认取消并要求明确确认；确认后在原名称上安全替换，失败会回滚，避免界面显示成功但 Runtime 仍加载旧版本。
- **CLI 定时任务的 prompt 文件更新更准确**：新增和更新统一校验 prompt 与文件输入，空内容、歧义输入或没有实际效果的修改会在写入前明确失败。
